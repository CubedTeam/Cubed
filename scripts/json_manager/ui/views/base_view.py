...
class BaseResourceView(ft.Column):
    ...
    def __init__(self, page: ft.Page) -> None:
        ...
        self.mode_toggle = ft.SegmentedButton(
            selected=["form"],
            segments=[
                ft.Segment(value="form", label=ft.Text("Form"), icon=ft.Icon(ft.Icons.EDIT_NOTE)),
    ...
    def _action_buttons(self) -> list[ft.Control]:
        return [
            ft.FilledTonalButton("Save", icon=ft.Icons.SAVE_OUTLINED, on_click=self._on_save),
            ft.OutlinedButton(
                "Delete", icon=ft.Icons.DELETE_OUTLINE, on_click=self._onDelete,
                disabled=self.selected_name is None,
            ),
            ft.OutlinedButton(
                "Duplicate", icon=ft.Icons.CONTENT_COPY, on_click=self._onDuplicate,
    ...
    def _current_payload(self) -> dict[str, Any] | None:
        if self._mode() == "raw":
            ...
            try:
                ...
                snack(self.page_ctx, f"JSON parse failed: {e}", "error")
                return None
    ...
    def _on_save(self, _e: ft.ControlEvent) -> None:
        ...
        try:
            ...
            snack(self.page_ctx, f"Save failed: {ex}", "error")
            return
        self.selected_name = saved_name
        self.current_data = data
        self.refresh_list()
        snack(self.page_ctx, "Saved " + saved_name, "ok")
    ...
    def _onDelete(self, _e: ft.ControlEvent) -> None:
        ...
        def _do_delete():
            ...
            snack(self.page_ctx, f"Deleted {self.selected_name}", "ok")
            self.selected_name = None
            self.current_data = self.blank_form_data() or {}
            self.refresh_list()
            self._render("form")

        confirm(self.page_ctx, "Confirm delete", f"Delete {self.selected_name}?", _do_delete)
